...
namespace ER {
    HWND FindERWindow() {
        ...
        if (window == NULL) {
            Logger::log("Failed to find \"ELDEN RING™\" window, trying to get via GetForegroundWindow",
                        LogLevel::Warning);
	...
	try
	{
        ...
        if (D3D12Module == NULL || DXGIModule == NULL) {
            Logger::log(
                "Failed to get DX modules: D3D12Module - " + std::to_string(D3D12Module != NULL) + "DXGIModule - " +
                std::to_string(DXGIModule != NULL), LogLevel::Error);
            DeleteWindow();
            return false;
        }
        ...
        ID3D12Device *Device;
        auto test = ((HRESULT(__stdcall*)(IUnknown *, D3D_FEATURE_LEVEL, const IID &, void **)) (D3D12CreateDevice))(
        ...
        Logger::log("Alloc D3DWindow method tables");
    ...
    D3D12TextureData initD3D12Texture(const TextureFileData &textureFileData, ID3D12Device *device,
                                      ID3D12DescriptorHeap *descriptor, int descriptor_index) {
        ...
        device->CreateCommittedResource(&props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COPY_DEST, NULL,
        ...
        if (!m_Init) {
            ...
            m_BuffersCounts = sc_desc.BufferCount; {
                ...
                if (pDevice->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&m_srvDescriptorHeap)) != S_OK) {
                    pDevice->Release();
                    pSwapChain3->Release();
                    Logger::log("Failed to create descriptor heap type D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV",
                                LogLevel::Warning);
                    return;
                }
                ...
                if (textureFileDataLoaded) {
                    ...
                    auto &&bossBarBorderTextureData = initD3D12Texture(bossBarBorderFileData, pDevice,
                                                                       m_srvDescriptorHeap, 2);
                    auto &&bossBarTextureData = initD3D12Texture(bossBarFileData, pDevice, m_srvDescriptorHeap, 3);
                    auto &&entityBarBorderTextureData = initD3D12Texture(
                        entityBarBorderFileData, pDevice, m_srvDescriptorHeap, 4);
                    auto &&entityBarTextureData = initD3D12Texture(entityBarFileData, pDevice, m_srvDescriptorHeap, 5);

                    if (!(bossBarBorderTextureData.dx12Resource && bossBarTextureData.dx12Resource &&
                          entityBarBorderTextureData.dx12Resource && entityBarTextureData.dx12Resource)) {
                        pDevice->Release();
                        pSwapChain3->Release();
                    ...
                    g_postureUI->bossBarTexture = TextureBar(
                        TextureData((ImTextureID) bossBarBorderTextureData.gpuHandle.ptr,
                                    (float) bossBarBorderTextureData.width, (float) bossBarBorderTextureData.height),
                        TextureData((ImTextureID) bossBarTextureData.gpuHandle.ptr, (float) bossBarTextureData.width,
                                    (float) bossBarTextureData.height)
                    );

                    g_postureUI->entityBarTexture = TextureBar(
                        TextureData((ImTextureID) entityBarBorderTextureData.gpuHandle.ptr,
                                    (float) entityBarBorderTextureData.width,
                                    (float) entityBarBorderTextureData.height),
                        TextureData((ImTextureID) entityBarTextureData.gpuHandle.ptr,
                                    (float) entityBarTextureData.width, (float) entityBarTextureData.height)
                    );
                ...
                if (pDevice->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&m_rtvDescriptorHeap)) != S_OK) {
                    ...
                    Logger::log("Failed to create descriptor heap type D3D12_DESCRIPTOR_HEAP_TYPE_RTV",
                                LogLevel::Warning);
                    return;
                }
            ...
            for (IndexType i = 0; i < sc_desc.BufferCount; ++i) {
                if (pDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,
                                                    IID_PPV_ARGS(&m_CommandAllocators[i])) != S_OK) {
                    pDevice->Release();
                    pSwapChain3->Release();
                    m_srvDescriptorHeap->Release();
                    for (UINT j = 0; j < i; ++j) {
                    ...
                    Logger::log(
                        "Failed to create command allocator D3D12_COMMAND_LIST_TYPE_DIRECT buffer idx: " +
                        std::to_string(i), LogLevel::Warning);
                    return;
                }
            }

            if (pDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_CommandAllocators[0], NULL,
            ...
            if (!ImGui_ImplDX12_Init(pDevice, sc_desc.BufferCount, DXGI_FORMAT_R8G8B8A8_UNORM, NULL,
            ...
            mainViewport->PlatformHandleRaw = programData->m_GameWindow;
    ...
    bool D3DRenderer::InitWindow() {
        ...
        WindowHwnd = CreateWindow(WindowClass.lpszClassName, "DirectX Window",
                                  WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_THICKFRAME, 0, 0, 100, 100, NULL, NULL,
